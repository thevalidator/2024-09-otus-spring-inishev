...
//        dbBooks = getDbBooks(dbAuthors, dbGenres);
    ...
    void shouldReturnCorrectBookById(Book book) {
        Book expectedBook = testEntityManager.find(Book.class, book.getId());
    ...
    void shouldReturnCorrectBooksList() {
        var b1 = testEntityManager.find(Book.class, 1);
        var b2 = testEntityManager.find(Book.class, 2);
        var b3 = testEntityManager.find(Book.class, 3);
    ...
    @DisplayName("должен выбросить исключение EntityNotFoundException, если не обновилось ни одной строчки")
    @Test
    void shouldThrowEntityNotFoundExceptionWhenNoRowsUpdated() {
        var expectedBook = new Book(999999999L, "BookTitle_10500", dbAuthors.get(2),
                List.of(dbGenres.get(4), dbGenres.get(5)));

        assertThat(bookRepository.findById(expectedBook.getId()))
                .isNotPresent();

        assertThrows(EntityNotFoundException.class, () -> bookRepository.save(expectedBook));
    }
